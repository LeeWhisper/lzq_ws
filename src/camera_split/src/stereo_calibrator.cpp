...
boost::format left_imgs("/home/lzq/lzq_ws/src/camera_split/left_img/left%03d.jpg");
boost::format right_imgs("/home/lzq/lzq_ws/src/camera_split/right_img/right%03d.jpg");
const int frameNumber = 35;                             //相机标定时需要采用的图像张数
const int squareSize = 22;                              //标定板黑白格子的大小 单位mm
...
0 0  1
*/
Mat cameraMatrixL = (Mat_<double>(3, 3) << 436.04221,   0.     , 304.92764,
                                           0.       , 437.4338 , 260.25371,
                                           0.       ,   0.     ,   1.     );                                                                           //这时候就需要你把左右相机单目标定的参数给写上
//获得的畸变参数
Mat distCoeffL = (Mat_<double>(5, 1) << 0.152835, -0.141151, 0.002608, -0.005254, 0.000000);
/*
事先标定好的右相机的内参矩阵
fx 0 cx
0 fy cy
0 0  1
*/
Mat cameraMatrixR = (Mat_<double>(3, 3) << 433.2497 ,   0.     , 328.40851,
           0.     , 433.78436, 232.00682,
           0.     ,   0.     ,   1.     );
Mat distCoeffR = (Mat_<double>(5, 1) << 0.164184, -0.157772, 0.001051, 0.000945, 0.000000);
...
int main(int argc, char* argv[])
{
    ...
    while (goodFrameCount < frameNumber)
    {
        char filename[100];
        /*读取左边的图像*/
        rgbImageL = imread((left_imgs % goodFrameCount).str(), 1);
        cvtColor(rgbImageL, grayImageL, CV_BGR2GRAY);

        /*读取右边的图像*/
        rgbImageR = imread((right_imgs % goodFrameCount).str(), 1);
    ...
    */
    ...
    标定摄像头
    由于左右摄像机分别都经过了单目标定
    所以在此处选择flag = CALIB_USE_INTRINSIC_GUESS, 根据指定的FLAG优化一些或全部的内在参数。初始值是由用户提供
    */
    ...
    */
    ...
    imshow("Rectify Before ImageL", rectifyImageL);
    imshow("Rectify Before ImageR", rectifyImageR);
