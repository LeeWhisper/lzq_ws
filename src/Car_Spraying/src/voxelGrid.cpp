...
int main()
{
	...
	reader.read("/home/lzq/lzq_ws/pointcloud/3.1/Hf_De.pcd", *cloud);
	cout << "\t\t<读入点云信息>\n"
		 << *cloud << endl;

	// std::cout << "R: " << (int)cloud->points[1].r << std::endl;
	// std::cout << "G: " << (int)cloud->points[1].g << std::endl;
	// std::cout << "B: " << (int)cloud->points[1].b << std::endl;
	/// 体素滤波器点云下采样
	cout << "->正在体素下采样..." << endl;
	startTime = clock();
	pcl::VoxelGrid<pcl::PointXYZRGB> vg; // 创建滤波器对象
	vg.setInputCloud(cloud);			 // 设置待滤波点云
	vg.setLeafSize(0.05f, 0.05f, 0.05f); // 设置体素大小，单位是m
	vg.filter(*cloud_filtered);			 // 执行滤波，保存滤波结果于cloud_filtered

	/// 保存下采样点云
	cout << "->正在保存下采样点云..." << endl;
	pcl::PCDWriter writer;
	writer.write("/home/lzq/lzq_ws/pointcloud/3.1/Hf_De_VG.pcd", *cloud_filtered, true);
	cout << "\t\t<保存点云信息>\n"
		 << *cloud_filtered << endl;
	endTime = clock();
