...
int main(int argc, char **argv)
{
    ...
    // if (pcl::io::loadPCDFile<pcl::PointXYZRGB>("/home/yxg/pcl/pcd/mid.pcd",*cloud) == -1)
    if (pcl::io::loadPCDFile<pcl::PointXYZRGB>("/home/lzq/lzq_ws/pointcloud/11.30/HR+sub+De_1.pcd", *cloud) == -1)
    {
    ...
    pcl::KdTreeFLANN<pcl::PointXYZRGB> kdtree; // 创建一个快速k近邻查询,查询的时候若该点在点云中，则第一个近邻点是其本身
    ...
    pcl::NormalEstimation<pcl::PointXYZRGB, pcl::Normal> normEst; // 其中pcl::PointXYZRGB表示输入类型数据，pcl::Normal表示输出类型,且pcl::Normal前三项是法向，最后一项是曲率
    ...
    //  pcl::PointCloud<pcl::PointXYZRGB> boundPoints;
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr boundPoints(new pcl::PointCloud<pcl::PointXYZRGB>);
    pcl::PointCloud<pcl::PointXYZRGB> noBoundPoints;
    ...
    pcl::io::savePCDFileASCII("/home/lzq/lzq_ws/pointcloud/11.30/boudary_1.pcd", *boundPoints);
    ...
    viewer->addPointCloud<pcl::PointXYZRGB>(boundPoints, "sample cloud2", v2);
    viewer->addCoordinateSystem(1.0);
